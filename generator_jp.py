...
src_dir='.'
target_dir=os.path.join(src_dir, 'generated_jp/')
...
reference_dir='Reference/api_jp/'
tutorials_dir='Tutorials_jp/'
...
def build_reference(reference_dir, to_update, env, build_images):
    ...
    reference_template = env.get_template("reference_item_template_jp.jinja")
...
def build_reference_index(reference_dir, env):
    ...
    index_template = env.get_template('reference_index_template_jp.jinja')
    with open(os.path.join('generated_jp/reference', 'index.html'), 'w') as tfile:
        tfile.write(clean_html(index_template.render(elements=elements)))
    print_success('success!')

def build_tutorials(env):
    print('Building tutorials')
    item_template = env.get_template('tutorial_item_template_jp.jinja')
    index_template = env.get_template('tutorial_index_template_jp.jinja')
...
def build_examples(env):
    ...
    examples_template = env.get_template('examples_index_jp.jinja')
...
def build_cover(env):
    ...
    cover_template = env.get_template('index_jp.jinja')
    with open(os.path.join('generated_jp', 'index.html'), 'w') as tfile:
...
def build(build_images, to_update):
    ...
    reference_dir = os.path.join(src_dir, 'Reference', 'api_jp')
    tutorials_dir = os.path.join(src_dir, 'Tutorials_jp')
